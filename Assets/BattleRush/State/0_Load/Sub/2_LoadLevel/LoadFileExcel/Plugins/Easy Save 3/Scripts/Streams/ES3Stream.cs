...
namespace ES3Internal
{
	public static class ES3Stream
	{
		public static Stream CreateStream(ES3Settings settings, ES3FileMode fileMode)
		{
			...
			{
                // Debug.LogError("create stream resources: " + settings.path + "; " + settings.FullPath);
				if(!isWriteStream)
				{
					var resourcesStream = new ES3ResourcesStream(settings.FullPath);
                    if (resourcesStream.Exists)
                    {
                        // Debug.LogError("resourcesStream exist: " + settings.path + "; " + settings.FullPath);
                        stream = resourcesStream;
                    }
					else
					{
                        // Debug.LogError("resourcesStream not exist: " + settings.path + "; " + settings.FullPath);
                ...
                // Debug.LogError("create stream resources: " + stream);
            }
